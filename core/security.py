...
def create_access_toke(data: dict) -> str:
    to_encode = data.copy()
    to_encode.update(
        {
...
def decode_access_toke(token: str) -> dict:
    try:
        encode_jwt = jwt.decode(token=token, key=SECRET_KEY, algorithms=[ALGORITM])
    except jwt.JWSError:
        return None
    return encode_jwt
...
class JWTBearer(HTTPBearer):
    ...
    async def __call__(self, request: Request):
        ...
        if credentials:
            token = decode_access_toke(credentials.credentials)
            if token is None:
                raise exp
